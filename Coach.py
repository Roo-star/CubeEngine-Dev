...
<<<<<<< HEAD
            ...
            draw_examples = [ex for ex in trainExamples if abs(ex[2]) < 0.01]
            orphan_examples = [
                ex for ex in trainExamples
                if not (ex[2] > 0.5 or ex[2] < -0.5 or abs(ex[2]) < 0.01)
            ]

            # region agent log
            _debug_log('H3', 'Coach.py:learn', 'training example bucket counts', {
                'total': len(trainExamples),
                'win': len(win_examples),
                'lose': len(lose_examples),
                'draw': len(draw_examples),
                'orphan': len(orphan_examples),
...
>>>>>>> origin/dev
            ...
            weighted_examples = win_examples + lose_examples * 3 + draw_examples

            # 3. 将新的、加权过的数据打乱再传给网络
            shuffle(weighted_examples)
            ...
            self.nnet.train(weighted_examples)

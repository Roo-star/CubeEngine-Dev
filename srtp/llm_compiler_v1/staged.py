...
SCHEMAS = {'rule_ir':'ir_v2/rule-ir-v2.schema.json', 'asset_ir':'asset_ir_v2/asset-ir-v2.schema.json',
...
def _execute_stage(slot, documents, root, tests, *, spatial=False):
    ...
    if slot == 'scene_ir':
        ...
        return [{'scene_nodes':len(graph.nodes), 'compiled':True,'replayed_frames':frames,'behavior_traces':len(tests)}, *validate_volume(graph)]
...
def run_stages(compiler, *, package, evidence, documents, job_id, design_intent=None, source_manifest_hash=None):
    ...
    for asset in workspace.assets:
        source_hashes[asset['path']] = hashlib.sha256((root/asset['path']).read_bytes()).hexdigest()
    ...
    engine_files += [Path(__file__).resolve().parents[1]/p for p in
        ('ir_v2/runtime.py','ir_v2/sequence_functions.py','session_random.py','llm_compiler_v1/behavior_runtime.py','scene_presentation.py','volume_layout.py','presentation_patterns.py','ursina_scene_backend.py','source_visuals.py','sprite_geometry.py',
         'bundle_assets.py','runtime_assets.py','system_fonts.py','ir_contracts.py','input_adapter_contract.py','asset_ir_v2/recipe_contracts.py',
         'scene_ir_v2/component_contracts.py','ir_v2/expression_contracts.py','asset_ir_v2/compiler.py','asset_ir_v2/asset_ir.py','visual_timeline.py','ir_v2/command_contracts.py','ir_v2/rule_ir.py','ir_v2/types.py','scene_ir_v2/compiler.py','scene_ir_v2/scene_ir.py',
    ...
    intent_key = {key:value for key,value in (design_intent or {}).items()
                  if key not in ('intent_id','conversation_id','turn_id')}
    transport_identity = getattr(compiler.client, 'cache_identity', {})
    if not isinstance(transport_identity, dict):
        transport_identity = {}
    input_identity = {'source':evidence['source_package_hash'],'files':source_hashes,
        'model_configuration':transport_identity,
        'base':{k:d['content_hash'] for k,d in documents.items()},'intent':intent_key,
    ...
    if cache_path and cache_path.is_file():
        try:
            ...
            if stored.get('signature') == signature or stored.get('input_signature') == input_signature:
    ...
    for slot in ORDER:
        ...
        for attempt in range(compiler.max_repairs + 1 + int(slot in cached)):
            ...
            try:
                ...
                result.trace.append({'stage':slot,'attempt':attempt+1,'passed':False,'diagnostics':list(feedback)})

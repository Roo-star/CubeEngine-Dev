"""Revision-safe RFC 6902 transactions for Asset IR v2."""
...
from .asset_ir import canonical_asset_ir_hash, seal_asset_ir, validate_asset_ir


class AssetIRPatchError(ValueError):
    pass
...
def apply_asset_ir_patch(document: Mapping[str, Any], proposal: Mapping[str, Any]) -> Dict[str, Any]:
    if not isinstance(document, Mapping) or not isinstance(proposal, Mapping):
        raise AssetIRPatchError("document and patch proposal must be objects")
    _validate_base(document, proposal)
    operations = proposal.get("operations")
    if not isinstance(operations, list) or not operations:
        raise AssetIRPatchError("patch proposal requires at least one operation")
    if not isinstance(proposal.get("evidence"), list) or not proposal.get("evidence"):
        raise AssetIRPatchError("patch proposal requires source or designer evidence")
    if not isinstance(proposal.get("assumptions"), list) or not isinstance(proposal.get("unresolved"), list):
        raise AssetIRPatchError("patch proposal assumptions and unresolved must be arrays")

    staged: Any = deepcopy(dict(document))
    for index, operation in enumerate(operations):
        try:
            staged = _apply_operation(staged, operation)
        except AssetIRPatchError as exc:
            raise AssetIRPatchError("operation {0}: {1}".format(index, exc))
    if not isinstance(staged, dict):
        raise AssetIRPatchError("Asset IR patch cannot replace the document root")
    staged.setdefault("unresolved", []).extend(deepcopy(proposal["unresolved"]))
    provenance = staged.setdefault("provenance", {})
    if not isinstance(provenance, dict):
        raise AssetIRPatchError("patched provenance must remain an object")
    history = provenance.setdefault("patch_history", [])
    if not isinstance(history, list):
        raise AssetIRPatchError("provenance.patch_history must be an array")
    history.append({
    ...
    staged["content_hash"] = ""
    errors = [item for item in validate_asset_ir(staged) if item.severity == "error"]
    if errors:
        first = errors[0]
        raise AssetIRPatchError("patched Asset IR is invalid at {0}: {1}".format(first.path, first.message))
    return seal_asset_ir(staged)
...
def _validate_base(document: Mapping[str, Any], proposal: Mapping[str, Any]) -> None:
    if any(key not in proposal for key in ("document_id", "base_revision", "base_content_hash")):
        raise AssetIRPatchError("patch proposal is missing base identity")
    if proposal["document_id"] != document.get("document_id"):
        raise AssetIRPatchError("patch document_id does not match")
    if isinstance(proposal["base_revision"], bool) or not isinstance(proposal["base_revision"], int):
        raise AssetIRPatchError("base_revision must be an integer")
    if proposal["base_revision"] != document.get("revision"):
        raise AssetIRPatchError("patch base revision is stale")
    current_hash = document.get("content_hash")
    if not isinstance(current_hash, str) or not current_hash:
        raise AssetIRPatchError("patch base must be a sealed Asset IR document")
    if current_hash != canonical_asset_ir_hash(document):
        raise AssetIRPatchError("patch base content hash is invalid")
    if proposal["base_content_hash"] != current_hash:
        raise AssetIRPatchError("patch base content hash is stale")


def _apply_operation(root: Any, operation: Any) -> Any:
    if not isinstance(operation, Mapping):
        raise AssetIRPatchError("operation must be an object")
    name = operation.get("op")
    if name not in ("add", "remove", "replace", "move", "copy", "test"):
        raise AssetIRPatchError("unsupported RFC 6902 operation: {0}".format(name))
    if not isinstance(operation.get("path"), str):
        raise AssetIRPatchError("operation path must be a JSON Pointer")
    tokens = _tokens(operation["path"])
    _reject_protected(tokens)
    if not tokens:
        raise AssetIRPatchError("document-root operations are not allowed")
    if name == "test":
        if "value" not in operation or not _json_equal(_get(root, tokens), operation["value"]):
            raise AssetIRPatchError("test operation failed")
        return root
    if name in ("copy", "move"):
        if not isinstance(operation.get("from"), str):
            raise AssetIRPatchError("copy/move operation requires from")
        source_tokens = _tokens(operation["from"])
        _reject_protected(source_tokens)
        if not source_tokens:
            raise AssetIRPatchError("document-root copy/move is not allowed")
        if name == "move" and tokens[:len(source_tokens)] == source_tokens and len(tokens) > len(source_tokens):
            raise AssetIRPatchError("cannot move a value into its own descendant")
    ...
    if name == "add":
        if "value" not in operation:
            raise AssetIRPatchError("add operation requires value")
        _add(root, tokens, deepcopy(operation["value"]))
    elif name == "remove":
        _remove(root, tokens)
    elif name == "replace":
        if "value" not in operation:
            raise AssetIRPatchError("replace operation requires value")
...
def _tokens(pointer: str) -> Tuple[str, ...]:
    ...
    if not pointer.startswith("/"):
        raise AssetIRPatchError("path is not a JSON Pointer")
    result = []
        ...
        while index < len(token):
            if token[index] != "~":
                ...
                if index + 1 >= len(token) or token[index + 1] not in ("0", "1"):
                    raise AssetIRPatchError("invalid JSON Pointer escape")
...
def _reject_protected(tokens: Sequence[str]) -> None:
    if tokens and tokens[0] in _PROTECTED_ROOTS:
        raise AssetIRPatchError("patch cannot edit protected root field: {0}".format(tokens[0]))
    ...
    for token in tokens:
        if isinstance(current, Mapping):
            if token not in current:
                raise AssetIRPatchError("path does not exist")
            current = current[token]
        elif isinstance(current, list):
            current = current[_array_index(token, len(current), False)]
        else:
            raise AssetIRPatchError("path traverses a scalar value")
    return current
...
def _add(root: Any, tokens: Sequence[str], value: Any) -> None:
    ...
    if isinstance(parent, MutableMapping):
        ...
        raise AssetIRPatchError("add target parent is not a container")
...
def _remove(root: Any, tokens: Sequence[str]) -> Any:
    ...
    if isinstance(parent, MutableMapping):
        if token not in parent:
            raise AssetIRPatchError("remove path does not exist")
        return parent.pop(token)
    if isinstance(parent, MutableSequence):
        return parent.pop(_array_index(token, len(parent), False))
    raise AssetIRPatchError("remove target parent is not a container")
...
def _replace(root: Any, tokens: Sequence[str], value: Any) -> None:
    ...
    if isinstance(parent, MutableMapping):
        if token not in parent:
            raise AssetIRPatchError("replace path does not exist")
        parent[token] = value
    elif isinstance(parent, MutableSequence):
        parent[_array_index(token, len(parent), False)] = value
    else:
        raise AssetIRPatchError("replace target parent is not a container")
...
def _array_index(token: str, length: int, allow_end: bool) -> int:
    if not token or (token.startswith("0") and token != "0") or not token.isdigit():
        raise AssetIRPatchError("invalid array index")
    index = int(token)
    maximum = length if allow_end else length - 1
    if index < 0 or index > maximum:
        raise AssetIRPatchError("array index is outside bounds")
    return index
